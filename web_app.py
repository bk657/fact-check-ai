...
try:
    ...
    st.error("❌ 필수 키(API Key, DB Key, Password)가 설정되지 않았습니다. Streamlit Secrets를 확인해주세요.")
    st.stop()
...
def run_forensic_main(url):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            # 🌟 [중요] 실질적 침묵: 뉴스 0건이거나, 있어도 관련성 20% 미만
            ...
            if silent_echo_penalty > 0:
                st.error(f"🔇 **침묵의 메아리(Silent Echo) 경고**: 자극적인 주장('위독', '사망' 등)을 하고 있으나, 이를 뒷받침할 제도권 언론 보도가 전무합니다. 가짜뉴스일 확률이 매우 높습니다.")
            ...
            with col2:
                ...
                if is_official:
                    ...
                    result_text += "🔇 **'바이든 위독'과 같은 자극적 주장을 하고 있으나, 이를 뒷받침할 언론 보도가 없거나 관련성이 낮습니다(Silent Echo).** 가짜뉴스일 확률이 매우 높습니다. "
