...
# --- [1. 시스템 설정] ---
st.set_page_config(page_title="Fact-Check Center v47.3 (Target Fix)", layout="wide", page_icon="⚖️")
...
def loading_seq(level):
    with st.status("🕵️ Forensic Core v47.3 가동...", expanded=True) as s:
        st.write(f"🧠 Intelligence Level: {level}")
        st.write("📝 자막 키워드 심층 분석 중...")
...
# 🌟 [요청하신 기능] 자막에서 많이 나온 키워드 추출
def get_top_keywords_from_transcript(text, top_n=3):
    nouns = extract_nouns(text)
    if not nouns: return []
    return [word for word, count in Counter(nouns).most_common(top_n)]

# 🌟 [요청하신 기능] 쿼리 생성 시 자막 키워드 반영
def generate_hybrid_query(title, tags, transcript_text):
    # 1. 기본: 제목 + 태그
    base_text = title + " " + " ".join([t.replace("#","") for t in tags])
    base_nouns = extract_nouns(base_text)
    
    # 2. 자막에서 뽑은 TOP 3 키워드
    transcript_keywords = get_top_keywords_from_transcript(transcript_text)
    
    # 3. 결합 (중복 제거)
    final_keywords = []
    # VIP/Vital 우선 추가
    for w in base_nouns:
        if w in VIP_ENTITIES or w in VITAL_KEYWORDS:
            final_keywords.append(w)
    
    # 자막 키워드 추가
    for w in transcript_keywords:
    ...
    return " ".join(final_keywords[:4]) # 최대 4단어
...
# --- [App] ---
st.title("⚖️ Triple-Evidence Intelligence Forensic v47.3")
...
try:
    ...
    if not df.empty:
        if st.session_state["is_admin"]:
            ...
            df = df[cols]
            ed = st.data_editor(df, column_config={"Delete": st.column_config.CheckboxColumn("삭제", default=False)}, disabled=["id","video_title","fake_prob","keywords"], hide_index=True, use_container_width=True)
            to_del = ed[ed.Delete]
            if not to_del.empty:
