...
def get_gemini_search_keywords(title, transcript):
    ...
    [INPUT] Title: {title} / Transcript: {full_context}
    [RULES]
    1. Ignore clickbait (e.g., 'Shocking', 'Vlog').
    2. FIND: Drug names (e.g., Phentermine, Butterfly Pill), Crimes, Medical terms.
    3. Output: JUST THE QUERY.
    """
...
def get_gemini_verdict(title, transcript, news_items):
    ...
    result = call_gemini_with_retry(GOOGLE_API_KEY_B, target_model, prompt)
    
    if "ERROR" in result:
        return 50, f"AI 추론 실패: {result}"
        
    try:
        res_json = json.loads(result)
        return res_json['score'], res_json['reason']
    except:
        return 50, "JSON 파싱 오류 (AI 응답 불량)"
