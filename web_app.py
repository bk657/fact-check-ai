...
def call_triple_survivor(prompt, is_json=False):
    ...
    for model_name in MISTRAL_MODELS:
        try:
            ...
            logs.append(f"❌ Mistral Failed ({model_name})")
            time.sleep(0.2)
            continue

    # Phase 2: Gemini Key A
    ...
    # Phase 3: Gemini Key B
    logs.append("⚠️ Key A 전멸 -> Gemini Key B 투입")
...
def get_hybrid_search_keywords(title, transcript):
    ...
    prompt = f"""You are a Fact-Check Investigator. Input: {title}, {context_data}. Generate 3 Google News queries. Output JSON: {{ "queries": ["query1", "query2", "query3"] }}"""
    ...
    if parsed and 'queries' in parsed: return parsed['queries'], f"✨ {model_used}"
    fallback_q = result_text.strip().replace('"', '') if result_text else title
    return [fallback_q], f"⚠️ {model_used}"
...
def deep_verify_news(video_summary, news_url, news_snippet):
    ...
    prompt = f"""Compare: {video_summary[:2000]} vs {evidence_text}. Match(90-100), Related(40-60), Mismatch(0-10). Output JSON: {{ "score": <int>, "reason": "<short korean reason>" }}"""
    result_text, model_used, logs = call_triple_survivor(prompt, is_json=True)
...
def get_hybrid_verdict_final(title, transcript, verified_news_list):
    news_summary = "".join([f"- News: {i['뉴스 제목']} (Score: {i['최종 점수']}, Reason: {i['분석 근거']})\n" for i in verified_news_list])
    prompt = f"""Judge this video: {title} / {transcript[:2000]}. Evidence: {news_summary}. Output JSON: {{ "score": <int>, "reason": "<korean explanation>" }}"""
    result_text, model_used, logs = call_triple_survivor(prompt, is_json=True)
...
def save_analysis(channel, title, prob, url, keywords, detail_json=None):
    try: 
        ...
        data = {
            ...
            "video_url": url, "keywords": keywords, "detail_json": json_str
        }
        ...
        if res.status_code == 200:
            ...
            for i in data.get('items', []):
                ...
                if 'textDisplay' in snippet:
                    items.append(str(snippet['textDisplay'])) # [안전장치] 문자열 강제 변환
...
def analyze_comment_relevance(comments, context_text):
    ...
    safe_comments = " ".join([str(c) for c in comments]) # [안전장치] 리스트 내부 요소 문자열 변환
...
def render_report_full_ui(prob, db_count, title, channel, data, is_cached=False):
    ...
    # [사용자가 원했던 증거 및 AI 판단 탭 복구]
    tab1, tab2, tab3 = st.tabs(["📝 증거 및 AI 판단", "📊 점수 산정표", "💬 여론 분석"])
    
    with tab1:
        st.subheader("📰 팩트체크 증거 (Evidence List)")
        ...
        st.divider()
        st.subheader("⚖️ AI 최종 판결 (Judgement)")
        st.info(f"**AI Judge Opinion:**\n{data.get('ai_reason', '판단 보류')}")
            
    with tab2:
        st.subheader("📊 신뢰도 점수 산정 내역")
...
def run_forensic_main(url):
    ...
    # [수정됨] 캐싱(DB 조회) 로직 완전히 제거 -> 무조건 새로 분석
    # cached_res = supabase.table("analysis_history")... (삭제됨)
        ...
        try:
            ...
            for i, q in enumerate(queries):
                ...
                if items:
                    news_items = items
                    final_query = q
                    st.session_state["debug_logs"].append(f"✅ News Found: '{q}'")
                    break
                else:
                    st.session_state["debug_logs"].append(f"⚠️ No news: '{q}'")
                    time.sleep(0.5)

            if not news_items: final_query = queries[0]
            query = final_query 
            ...
            for idx, item in enumerate(news_items[:3]):
                ...
                news_ev.append({"뉴스 제목": item['title'], "일치도": f"{status_icon} {ai_s}%", "최종 점수": f"{ai_s}%", "분석 근거": ai_r, "비고": f"[{src}]", "원문": real_url})
            ...
            report_data = {
                ...
                "score_breakdown": score_bd, "ts": ts, "fs": fs, "query": query, 
                "tags": ", ".join([str(t) for t in tags]) if tags else "", # [안전장치] 태그 리스트 문자열 변환
...
def generate_b2b_report_logic(df_history):
    ...
    for channel, group in grouped:
        ...
        for k in group['keywords']:
            if isinstance(k, list): kw_bucket.extend([str(i) for i in k])
            elif k: kw_bucket.append(str(k))
        ...
        results.append({"채널명": channel, "위험 등급": grade, "평균 가짜 확률": f"{int(avg_score)}%", "최고 가짜 확률": f"{int(max_score)}%", "분석 영상 수": f"{count}개", "주요 타겟": targets})
    ...
    if st.session_state["is_admin"]:
        ...
        if st.button("📊 리포트 생성 및 분석"):
            try:
                ...
                if not rpt.empty:
                    st.dataframe(rpt, column_config={"위험 등급": st.column_config.TextColumn("Risk Level", help="평균 가짜뉴스 확률 기반 등급"), "평균 가짜 확률": st.column_config.ProgressColumn("Avg Risk", format="%s", min_value=0, max_value=100)}, use_container_width=True, hide_index=True)
                    csv = rpt.to_csv(index=False).encode('utf-8-sig')
                    st.download_button("📥 리포트 엑셀(CSV) 다운로드", csv, "b2b_report.csv", "text/csv")
                else: st.info("분석할 데이터가 충분하지 않습니다.")
            except Exception as e: st.error(f"리포트 생성 실패: {e}")
        st.divider()
        st.subheader("🛠️ 시스템 상태 및 디버그 로그")
        st.write("**🤖 Triple Defense System Status:**")
        st.caption("1️⃣ Mistral Priority Chain"); st.code(", ".join(MISTRAL_MODELS))
        st.caption("2️⃣ Gemini Key A (Dynamic Scan)"); st.code("active")
        st.caption("3️⃣ Gemini Key B (Dynamic Scan)"); st.code("active")
        if "debug_logs" in st.session_state and st.session_state["debug_logs"]:
            st.write(f"**📜 최근 실행 로그 ({len(st.session_state['debug_logs'])}건):**")
            log_text = "\n".join(st.session_state["debug_logs"])
            st.text_area("Logs", log_text, height=300)
        else: st.info("실행된 로그가 없습니다.")
        if st.button("로그아웃"):
            st.session_state["is_admin"] = False
            st.rerun()
    else:
