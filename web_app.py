...
# --- [1. 시스템 설정] ---
st.set_page_config(page_title="Fact-Check Center v51.3 (Keyword Logic Patch)", layout="wide", page_icon="⚖️")
...
# VIP_ENTITIES는 더 이상 하드코딩하지 않고 로직으로 찾습니다.
...
# 🌟 [UPDATED] 로직 기반 키워드 추출 엔진 (Logic-Brain v1.0)
HONORIFICS = ['회장', '대통령', '의원', '대표', '장관', '박사', '교수', '감독', '선수', '씨', '배우', '가수', '개그맨', '방송인', '변호사', '검사', '판사', '후보']

def hunt_subject(title_text):
    words = title_text.split()
    # 1. 호칭 기반 추론
    for i, word in enumerate(words):
        for hon in HONORIFICS:
            if hon in word and i > 0:
                candidate = normalize_korean_word(words[i-1])
                if len(candidate) > 1: return candidate
    
    # 2. 문장 맨 앞 명사
    tokens = extract_meaningful_tokens(title_text)
    if tokens: return tokens[0]
    return ""

def hunt_action(title_text, transcript_text, subject):
    t_tokens = set(extract_meaningful_tokens(title_text))
    # 자막 앞부분 1000자만 사용하여 문맥 파악
    tr_tokens = set(extract_meaningful_tokens(transcript_text[:1000]))
    
    # 교집합 단어 중 주어가 아닌 가장 긴 단어 (구체적 행위일 확률 높음)
    common = t_tokens.intersection(tr_tokens)
    candidates = [w for w in common if w != subject and len(w) > 1]
...
def generate_revolutionary_query(title, hashtags, transcript):
    # 1. 주어 찾기
    subject = hunt_subject(title)
    
    # 2. 행위 찾기
    action = hunt_action(title, transcript, subject)
    
    # 3. 조립
    if subject and action:
        return f"{subject} {action}"
    elif subject:
        # 행위를 못 찾았으면 제목의 맨 뒤 단어(서술어) 사용
        last_word = normalize_korean_word(title.split()[-1])
        return f"{subject} {last_word}"
    ...
    with yt_dlp.YoutubeDL({'quiet': True, 'skip_download': True}) as ydl:
        try:
            ...
            with col2:
                ...
                st.caption(f"📡 수집: **{len(news_ev)}건** (검색어와 상태 불일치 시 0점 처리)")
...
# --- [UI Layout] ---
st.title("⚖️ Triple-Evidence Intelligence Forensic v51.3")
with st.container(border=True):
