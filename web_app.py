...
# [공통] 오뚜기 재시도 로직
def call_gemini_with_retry(api_key, model_name, prompt, is_json=False):
...
def get_gemini_verdict(title, transcript, news_items):
    ...
    result = call_gemini_with_retry(GOOGLE_API_KEY_B, target_model, prompt, is_json=True)
    ...
    if result:
        try:
            ...
            return 50, "JSON Parsing Error"
            
    return 50, "AI Inference Failed"
