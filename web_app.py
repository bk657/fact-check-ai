...
# --- [1. 시스템 설정] ---
st.set_page_config(page_title="Fact-Check Center v55.0 (TextRank)", layout="wide", page_icon="⚖️")
...
# --- [3. 핵심 분석 엔진 (TextRank Logic)] ---
# 🌟 [New] TextRank-Lite: 그래프 기반 중요도 산출 알고리즘
...
def extract_meaningful_tokens(text):
    ...
    noise = ['충격', '경악', '속보', '긴급', '오늘', '내일', '지금', '결국', '뉴스', '영상', '대부분', '이유', '왜', '있는', '없는', '하는', '것', '수', '등', '진짜', '정말', '너무', '그냥', '이제', '사실', '국민', '우리', '대한민국', '여러분', '그리고', '그래서', '그러나', '솔직히', '무슨', '어떤', '이런', '저런']
    tokens = [normalize_korean_word(w) for w in raw_tokens]
    return [t for t in tokens if t not in noise and len(t) > 1]

# 🌟 [New] TextRank 알고리즘 구현 (순수 파이썬)
def textrank_keywords(text, window_size=3, top_n=3):
    ...
    # 2. PageRank 알고리즘 적용 (점수 계산)
    scores = {word: 1.0 for word in graph}
    d = 0.85 # damping factor
    
    for _ in range(10): # 10번 반복 수렴
        new_scores = {}
        for node in graph:
            sum_score = 0
            for neighbor in graph[node]:
                sum_score += scores[neighbor] / len(graph[neighbor])
            new_scores[node] = (1 - d) + d * sum_score
        scores = new_scores
...
# 🌟 [Updated] TextRank 기반 하이브리드 쿼리 생성
def generate_smart_query(title, transcript):
    # 1. 주어(Subject) 찾기 - 제목 기반 (가장 정확함)
    subject = ""
    for vip in VIP_ENTITIES:
        if vip in title: subject = vip; break
        
    if not subject:
        # 호칭 기반 추론
        honorifics = ['회장', '의원', '대표', '대통령', '장관', '박사', '교수', '감독', '선수', '씨', '배우', '가수']
        title_split = title.split()
        for i, word in enumerate(title_split):
            for hon in honorifics:
                if hon in word and i > 0:
                    subject = normalize_korean_word(title_split[i-1])
                    
    # 2. 행위(Action) 찾기 - 자막 TextRank (문맥상 가장 중요한 단어)
    # 제목보다는 자막 전체의 흐름에서 '중심 단어'를 찾음
    tr_keywords = textrank_keywords(transcript, top_n=5)
    
    # 주어와 겹치지 않는 최상위 키워드 선택
    action = ""
    ...
    # 3. Fallback: 자막 분석 실패 시 제목 교집합 사용
    if not action:
    ...
    if subject and action:
        ...
        return f"{subject} {title.split()[-1]}"
    elif action:
        return action # 주어가 없으면 TextRank 1등 키워드 사용
    else:
...
def witty_loading_sequence(total):
    messages = [
        ...
        "📡 TextRank 알고리즘: 자막 문맥 연결 구조 계산 중...",
        "🔍 핵심 키워드 추론 및 팩트 교차 검증 중...", 
        "🚀 위성이 유튜브 본사 상공을 지나가는 중..."
    ]
    with st.status("🕵️ Context Merger v55.0 가동 중...", expanded=True) as status:
...
# --- [6. 메인 실행] ---
st.title("⚖️ Triple-Evidence Intelligence Forensic v55.0")
        ...
        with yt_dlp.YoutubeDL({'quiet': True}) as ydl:
            try:
                ...
                with col1:
                    ...
                    st.info(f"🎯 **AI 추론 검색어 (TextRank)**: {query}")
                    with st.container(border=True):
